...
namespace StoneMaster.Corel.UI
{
    public partial class StoneDockerView : UserControl
    {
        ...
        private bool _busy;
        private readonly string[] _stoneSizes = { "SS6", "SS8", "SS10", "SS12", "SS16", "SS20", "SS30", "SS34" };
        ...
        public StoneDockerView()
        {
            ...
            foreach (var size in _stoneSizes)
                StoneSizeBox.Items.Add(size);
            StoneSizeBox.SelectedItem = "SS10";
            foreach (var color in _palette)
            {
                ...
                PalettePanel.Children.Add(check);
            }
            UpdateDensityLabel();
        ...
        private void Mode_Changed(object sender, SelectionChangedEventArgs e)
        {
            if (ModeBox.SelectedItem is ComboBoxItem item)
            {
                ...
                if (item.Content.ToString() == "BUDGET")
                    StatusText.Text = "Bütçe modu: sonuç hedef bütçeye göre optimize edilir.";
            }
        }
        ...
        private async void AnalyzeColors_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                StatusText.Text = colors.Count == 0
                    ...
                    : "Önerilen renkler: " + string.Join(", ", colors.Take(6).Select(c => c.Name));
            }
            catch (Exception ex) { ShowError(ex); }
        ...
        private async void Preview_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var response = await _viewModel.PreviewAsync(progress);
                StatusText.Text = $"Önizleme hazır · {response.stone_count:N0} taş · {response.total_cost_tl:N2} TL";
            }
            catch (Exception ex) { ShowError(ex); }
            finally { SetBusy(false); }
        }
        ...
        private void Generate_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                MainPlugin.Corel.ApplyGeneration(
                    _viewModel.LastResponse,
                    _viewModel.CurrentBitmapContext,
                    renderPreview: true,
                    renderLaser: true,
                    clearPrevious: true);
        ...
        {
            _viewModel.StoneSize = StoneSizeBox.SelectedItem?.ToString() ?? "SS10";
            _viewModel.StoneSizes = new List<string> { _viewModel.StoneSize };
            ...
            _viewModel.PaletteColors = PalettePanel.Children.OfType<CheckBox>()
        ...
        private void EnsureImage()
        {
            if (string.IsNullOrWhiteSpace(_viewModel.ImagePath))
                UseSelection_Click(this, new RoutedEventArgs());
            if (string.IsNullOrWhiteSpace(_viewModel.ImagePath))
                throw new InvalidOperationException("Önce bir bitmap seçin veya görsel içe aktarın.");
        }
